...
rt <- search_tweets("flu", n = 10000, include_rts = FALSE, geocode = usa, retryonratelimit = T, max_id = m)
print(dim(rt))
...
save_as_csv(rt, file_name = "flu_latest.csv", prepend_ids = TRUE, na = "", fileEncoding = "UTF-8")
#View(unique(rt$location))
}

##ETL
#flu1 <- read.csv('flu_18k_1.csv', header = T, stringsAsFactors = F)
flu1 <- read.csv('flu_latest.csv', header = T, stringsAsFactors = F)
